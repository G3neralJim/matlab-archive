...
% Number of elements
ne = 2*n^2;
% Number of nodes (unkown nodes are (N-2)^2)
nc = N^2;

% Step
h = 1/n;

%% Assembly of Auxilary Matrices
% Coordinate matrix: Contains the position of each node in the domain
coords = zeros(nc,2);
for j = 1:N
  for i = 1:N
    coords((j-1)*N + i, :) = [(i-1)*h, (j-1)*h];
  end
end


% Local-to-global matrix: i = element, j = vertex (local)
% (i,j) = numbering of j-th vertex of the i-th element in global domain
...
for nx = 1:n
  for ny = 1:n
    l2g(k, :) = [numbering(ny,nx), numbering(ny, nx+1), numbering(ny+1, nx)];
    l2g(k+1, :) = [numbering(ny+1,nx+1), numbering(ny+1, nx), numbering(ny, nx+1)];
    k = k + 2;
  end
end
...
for e = 1:ne
  ...
  P1 = @(k,e) (1-k-e).*f(x(k,e),y(k,e));
  P2 = @(k,e) (k).*f(x(k,e),y(k,e));
  P3 = @(k,e) (e).*f(x(k,e),y(k,e));

  Fe(1) = 2 * Ae * integral2(P1, 0,1, 0, @(k) 1-k);
  Fe(2) = 2 * Ae * integral2(P2, 0,1, 0, @(k) 1-k);
  Fe(3) = 2 * Ae * integral2(P3, 0,1, 0, @(k) 1-k);
